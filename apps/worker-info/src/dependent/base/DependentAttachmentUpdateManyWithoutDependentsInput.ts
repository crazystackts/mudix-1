/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { DependentAttachmentWhereUniqueInput } from "../../dependentAttachment/base/DependentAttachmentWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class DependentAttachmentUpdateManyWithoutDependentsInput {
  @Field(() => [DependentAttachmentWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [DependentAttachmentWhereUniqueInput],
  })
  connect?: Array<DependentAttachmentWhereUniqueInput>;

  @Field(() => [DependentAttachmentWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [DependentAttachmentWhereUniqueInput],
  })
  disconnect?: Array<DependentAttachmentWhereUniqueInput>;

  @Field(() => [DependentAttachmentWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [DependentAttachmentWhereUniqueInput],
  })
  set?: Array<DependentAttachmentWhereUniqueInput>;
}

export { DependentAttachmentUpdateManyWithoutDependentsInput as DependentAttachmentUpdateManyWithoutDependentsInput };
