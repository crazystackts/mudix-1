...
export class EventServiceBase {
  ...
  async getCategory(parentId: number): Promise<PrismaCategory | null> {
    return this.prisma.event
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getLocation(parentId: number): Promise<PrismaLocation | null> {
    return this.prisma.event
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getUnit(parentId: number): Promise<PrismaUnit | null> {
    return this.prisma.event
      .findUnique({
        where: { id: parentId },
      })
