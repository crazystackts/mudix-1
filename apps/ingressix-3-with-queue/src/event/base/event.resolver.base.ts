...
export class EventResolverBase {
  ...
  async createEvent(@graphql.Args() args: CreateEventArgs): Promise<Event> {
    return await this.service.createEvent({
      ...
      data: {
        ...
        locations: {
          connect: args.data.locations,
        },

        units: {
          connect: args.data.units,
        },
      },
    });
  }
  ...
  async getLocations(
    ...
    const result = await this.service.getLocations(parent.id);
  ...
  async getUnits(@graphql.Parent() parent: Event): Promise<Unit | null> {
    const result = await this.service.getUnits(parent.id);
